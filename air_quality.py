...
try:
    while True:
        ...
        print(f'{aq_utils.aqi} | {aq_utils.elapsed_time} | Samples {aq_utils.sample_count} | Rel. err. {aq_utils.sensors_relative_error_percent}% | Spearman corr. {aq_utils.sensors_spearman_corr}%', flush=True)
        for i in range(aq_utils._serial_port_count):
            if aq_utils._pm1_cf1[i] and aq_utils._pm2_5_cf1[i] and aq_utils._pm10_cf1[i]:
                line = f'#{i} '
